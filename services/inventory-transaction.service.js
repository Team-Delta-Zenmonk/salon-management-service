...
exports.listTransactions = async (payload) => {
  ...
  if (search) {
    includeItem.where = {
      [Op.or]: [{ name: { [Op.iLike]: `%${search}%` } }, { brand: { [Op.iLike]: `%${search}%` } }],
    };
...
exports.getTransaction = async (payload) => {
  ...
  const transaction = await inventoryTransactionRepository.findOne({ uuid: params.uuid, salon_id: salon.id }, ["item"]);
  if (!transaction) {
...
exports.updateTransaction = async (payload) => {
  ...
  const oldNetChange =
    (transaction.received_quantity || 0) - (transaction.damaged_quantity || 0) - (transaction.returned_quantity || 0);
