...
exports.createBooking = async (payload) => {
  ...
  return await bookingRepository.handleManagedTransaction(async (transaction) => {
    ...
    if (existingBooking) {
      ...
      const isExpired =
        existingBooking.expires_at && existingBooking.expires_at < now;

      const sameSlot =
        new Date(existingBooking.booking_start_time).getTime() ===
          new Date(slot.start).getTime() &&
        new Date(existingBooking.booking_end_time).getTime() ===
          new Date(slot.end).getTime();

      const sameCart = existingBooking.total_price === cart.total_price;

      if (!isExpired && sameSlot && sameCart) {
        await bookingRepository.update({
          payload: {
            expires_at: new Date(Date.now() + 10 * 60 * 1000),
          },
          criteria: { id: existingBooking.id },
          options: { transaction },
        });
      ...
      await bookingRepository.update({
        payload: { status: BookingStatus.ENUM.CANCELLED },
        criteria: { id: existingBooking.id },
        options: { transaction },
      });
    }
    ...
    for (const plan of executionPlan) {
      const conflicts = await bookingServiceRepository.findAll({
        criteria: {
          ...
          start_time: { [Op.lte]: plan.end_time },
          end_time: { [Op.gte]: plan.start_time },
        },
        include: [
          {
            association: "booking",
            where: {
              [Op.and]: [
                {
                  status: {
                    [Op.notIn]: [BookingStatus.ENUM.CANCELLED],
                  },
                },
                {
                  [Op.or]: [
                    { status: { [Op.ne]: BookingStatus.ENUM.PENDING } },
                    {
                      [Op.and]: [
